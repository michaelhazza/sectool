...
5. **Measure** — per consuming repo, after mapping: run `npx tsx scripts/doc-read-audit.ts --since <ISO> --until <ISO>` over equal pre- and post-mapping event-time windows and compare `referenceContextTokensPerActiveSession`; corroborate with the `context-load:` lines (sections + approximate lines loaded) across a few representative builds. Cut or trim packs that don't pay back.
