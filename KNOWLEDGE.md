...
- **M1** — static-clone read token moved off argv into `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_0`/`GIT_CONFIG_VALUE_0` env (git 2.31+), so it's in `/proc/<pid>/environ` (uid-only) not `/proc/<pid>/cmdline` (world-readable). Env-building extracted to exported `cloneTokenEnv()` in `src/static/orchestrator.ts` and unit-tested. Chose GIT_CONFIG env over the `GIT_ASKPASS` `.cjs` helper the write path uses, to avoid a helper-file deployment dependency in the static-scan image. **Two git-clone token channels now exist and differ**: config-write uses GIT_ASKPASS, static-scan uses GIT_CONFIG env — both keep the token out of argv, but if you touch either, they are NOT the same mechanism.
  ...
  - **[2026-07-05] `GIT_ALLOW_PROTOCOL` transport pin.** Added `cloneAllowProtocol(gitUrl)` to the clone env: 'https' for every scheme except a literal `file://` URL (→ 'file'). Belt-and-braces so git refuses a command-executing `ext::` (or `ssh://`/`git://`) transport even if a config rewrite introduced one — on top of the https-only schema and the `--` argv separator. `file` is permitted ONLY for `file://` gitUrls, which never occur in production (schema forces https) and exist solely for the local no-network integration fixtures (`orchestrator.integration.test.ts` clones `file://` bare repos) — it enables local-repo reads, not command execution. Deriving from the URL scheme (rather than hardcoding 'https') is what keeps those fixtures working; anything non-`file:` collapses to 'https' = blocked.
