...
| OpenClaw-authored builds stop at `MERGE_READY`; merge needs the operator's code-owner approval | `references/runtime-roles.md`; `templates/default-branch-ruleset.json` + CODEOWNERS (GitHub-enforced); ADR-0015 | unapproved OpenClaw update/merge of the default branch | revert on main | one approval per OpenClaw build | Identity-scoped: Claude admin-credential merge flows are unchanged; the boundary is enforced by GitHub ruleset + CODEOWNERS, not prompt text |
