...
- [ ] [origin:audit:full:2026-07-02] [status:open] Allowlist host match is case-sensitive with no normalization (LOW/high-confidence). `src/live/gate.ts:111` compares `entry.host === hostname`; `new URL(...).hostname` is lowercased by Node, but allowlist entries permit mixed case (`src/schemas/allowlist.ts:isDnsName` allows `[a-zA-Z]`). An entry like `Example.com` therefore never matches `https://example.com` — a fail-closed correctness bug (the host is silently un-scannable), not a bypass. Recommended: lowercase `entry.host` at load/schema time. Schema/load change → pass-3.
