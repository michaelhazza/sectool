...
## Deferred from codebase audit — full — 2026-07-02

**Captured:** 2026-07-02T08:50:55Z
**Source:** audit branch `audit/full-2026-07-02` (FULL-mode audit; findings returned inline in the audit run, no standalone log file per active session policy). Two fixes applied in-branch (commit 512dd38); items below routed for later action.

- [x] [origin:audit:full:2026-07-02] [status:fixed 2026-07-02] Constrain `gitUrl` transport scheme in the repo schema (HIGH/med-confidence). FIXED on `audit/full-2026-07-02`: added `HttpsGitUrlSchema` in `src/schemas/targets.ts` (https-only `.refine`); ext::/file://ssh:// rejected at config-parse time; unit tests added in `schemas.test.ts`. `GIT_ALLOW_PROTOCOL` pinning in the clone spawn env is a further defense-in-depth step still worth adding but not required now that the scheme is pinned at both config-load and config-write parse boundaries. `src/schemas/targets.ts:16` types `gitUrl` as `z.string().url()`, which accepts `ext::`, `file://`, `ssh://` and other transports. `src/static/orchestrator.ts:defaultAcquireRepo` clones it with `git clone` — an `ext::` URL is arbitrary command execution on the scanner/CI host; `file://` is local-file exfiltration. The 2026-07-02 pass-2 fix added a `--` separator (blocks option injection) but does NOT restrict the scheme. Recommended: `.url().refine(u => /^https:\/\//i.test(u))` (or allow `https`+`ssh` explicitly), AND set `GIT_ALLOW_PROTOCOL=https` (or `-c protocol.ext.allow=never -c protocol.file.allow=never`) in the clone spawn env. Schema-contract change → intentionally pass-3. Note gitUrl is written via the 2FA-gated dashboard / PR, so exploitation needs config-write access, but this is exactly the defense-in-depth the token/2FA controls are meant to backstop.

- [x] [origin:audit:full:2026-07-02] [status:fixed 2026-07-04] Read-token leaks into git argv (MEDIUM/high-confidence). FIXED on `audit/full-2026-07-02`: `AUDIT_GITHUB_READ_TOKEN` now travels to `git clone` via `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_0`/`GIT_CONFIG_VALUE_0` env vars (git 2.31+), NOT `-c http.extraHeader=…` on argv — so the credential lands in /proc/<pid>/environ (uid-readable only) instead of /proc/<pid>/cmdline (world-readable), matching the GIT_ASKPASS protection level of the write path, and it no longer appears in the thrown error's `cmd` string. Env-building extracted to the exported pure `cloneTokenEnv(token)` and unit-tested in `orchestrator.test.ts`; the misleading comment is corrected. (Chose GIT_CONFIG env over the GIT_ASKPASS helper to stay self-contained — no dependency on deploying the `.cjs` helper into the static-scan image.) **PR-review follow-up (2026-07-05):** the first cut used a GLOBAL `http.extraHeader`, which git sends to whatever host `gitUrl` names — and the schema allows any https host, so a malicious registry `gitUrl` could exfiltrate the token. `cloneTokenEnv(token, gitUrl)` now (a) attaches the header only when the clone host is `github.com` and (b) URL-scopes it to that origin (`http.https://github.com/.extraHeader`), verified against `git config --get-urlmatch` (matches github.com, returns nothing for other hosts). Regression tests added for non-GitHub / look-alike hosts. GHE support (non-github.com host) is a trivial follow-up if ever needed. **Second PR-review pass (2026-07-05):** added `GIT_ALLOW_PROTOCOL` transport pinning to the clone env via `cloneAllowProtocol(gitUrl)` — 'https' for every scheme except a literal `file://` URL (which returns 'file', used only by the local integration fixtures; never occurs in production where the schema forces https). This blocks `ext::`/`ssh://`/`git://` at the transport layer even if a git config rewrite tried to introduce one, without breaking the file:// integration tests. Unit-tested. `src/static/orchestrator.ts:90-91` passes `AUDIT_GITHUB_READ_TOKEN` as `-c http.extraHeader=AUTHORIZATION: basic <base64>` — a full argv element visible via `ps`/`/proc/<pid>/cmdline` (base64 is trivially reversible). The code comment claims the credential "never appears in the process argument list" — factually wrong. This is the exact leak the config-write path deliberately avoids via `GIT_ASKPASS` (spec §7 HIGH-2 explicitly disallows `http.extraHeader` for the write token). Recommended: switch the static clone to the same `GIT_ASKPASS` channel `src/ui/config-git.ts` uses (or a `0600` credential-helper file), and correct the misleading comment. Real-world exposure is limited (single-tenant ephemeral CI runner), hence MEDIUM.

- [x] [origin:audit:full:2026-07-02] [status:fixed 2026-07-02] Login pre-step is not re-validated against the allowlist (MEDIUM/high-confidence). FIXED on `audit/full-2026-07-02` at the schema layer: `loginPath` is now `RootRelativePathSchema` (must start with a single `/`, no `//` prefix, no backslash) in `src/schemas/targets.ts`, so `target.url + loginPath` can no longer resolve to an off-allowlist host; unit tests added. NOTE: a belt-and-braces runtime re-check (`assertAllowlisted(loginUrl)` inside `establishSession`) is still worth adding for defense-in-depth since it would catch any future non-schema-validated call path — left open as a smaller follow-up. `src/live/auth.ts:263-264` builds `loginUrl = target.url.replace(/\/$/,'') + auth.loginPath` by string concatenation and issues a GET + credential-bearing POST to it via raw `fetch`, never re-checking the resulting host against the allowlist. A crafted `loginPath` (`z.string().min(1)`, no charset limit) such as `.evil.com/login` or `@evil.com/` makes the login POST — carrying real env credentials — target a non-allowlisted host, contradicting the surviving invariant "no network I/O to a host absent from the allowlist." Recommended: resolve `new URL(auth.loginPath, baseUrl)` and assert `resolved.hostname === target.hostname` (fail closed otherwise); constrain `loginPath` to a leading-`/` path in the schema. Config-admin-trusted input, so defense-in-depth, but it is a genuine gap in the central invariant on a credential-bearing request.

- [x] [origin:audit:full:2026-07-02] [status:fixed 2026-07-04] ZAP context `includePaths` uses the target URL as an unescaped regex (MEDIUM/med-confidence). FIXED on `audit/full-2026-07-02`: `buildZapAutomationYaml` (`src/live/scanners/zap.ts`) now regex-escapes `targetUrl` via a new `escapeRegex` helper before appending the trailing `.*`, so the URL's dots/slashes match literally and can no longer over-match a sibling host (`…com.*` → `…comX.evil.net`). Unit tests added asserting the escaped form and the absence of the vulnerable bare `host.*`. NOTE: still worth confirming against the Docker-only ZAP integration test that a scan of the real target still spiders as expected — the unit tests prove the pattern shape but not ZAP's runtime interpretation; flagged for the next Docker run. `src/live/scanners/zap.ts:buildZapAutomationYaml` emits `includePaths: ["<targetUrl>.*"]`; the URL is YAML-quoted but NOT regex-escaped, and the unescaped `.`/trailing `.*` over-match sibling hosts — e.g. `https://staging.example.com.*` matches `https://staging.example.comX.evil.net/...`. If an allowlisted target links/redirects off-host, ZAP may treat that host as in-scope and send active-scan traffic to it (findings are still filtered by the exact-host `isInScope`, but traffic egresses). Recommended: regex-escape `targetUrl` before appending the path suffix, and anchor the include pattern. `buildZapAutomationYaml` is a pure, unit-tested function; deferred because scope-confinement is security-critical and the change should be validated against the Docker-only ZAP integration test, not just unit tests.

- [x] [origin:audit:full:2026-07-02] [status:fixed 2026-07-02] Allowlist host match is case-sensitive with no normalization (LOW/high-confidence). FIXED on `audit/full-2026-07-02`: `src/live/gate.ts` now compares `entry.host.toLowerCase() === hostname`; unit tests added in `gate.test.ts`. (Normalizing at load/schema time as originally suggested is also fine but the gate is the single chokepoint, so normalizing there guarantees it regardless of how the allowlist was constructed.) `src/live/gate.ts:111` compares `entry.host === hostname`; `new URL(...).hostname` is lowercased by Node, but allowlist entries permit mixed case (`src/schemas/allowlist.ts:isDnsName` allows `[a-zA-Z]`). An entry like `Example.com` therefore never matches `https://example.com` — a fail-closed correctness bug (the host is silently un-scannable), not a bypass. Recommended: lowercase `entry.host` at load/schema time. Schema/load change → pass-3.

- [x] [origin:audit:full:2026-07-02] [status:fixed 2026-07-04] Repo `name` flows unescaped into a filesystem path (LOW/med-confidence). FIXED on `audit/full-2026-07-02`: `RepoSchema.name` is now `RepoNameSchema` (`^[A-Za-z0-9._-]+$`) in `src/schemas/targets.ts`, so `/` and `..` can never reach the mkdtemp clone-dir prefix; `defaultAcquireRepo` additionally sanitizes the prefix (`replace(/[^A-Za-z0-9._-]/g, '_')`) as belt-and-braces for any non-schema-validated call path. Unit tests added in `schemas.test.ts`. (Only `RepoSchema.name` was constrained — staging-target/finding names use different schemas and legitimately contain spaces.) `src/static/orchestrator.ts:84` builds the clone temp dir via `mkdtemp(join(tmpdir(), 'audit-clone-' + target.name + '-'))`; `name` is `z.string().min(1)` with no charset restriction, so `/` or `..` in a repo name is interpolated into the mkdtemp prefix path. Impact is bounded (mkdtemp needs an existing parent; cleanup only removes the created dir), but a repo-name charset constraint (`^[A-Za-z0-9._-]+$`) in the schema is the clean fix. Pass-3 (schema change).

- [x] [origin:audit:full:2026-07-02] [status:fixed 2026-07-04] Operational audit cache hash-chain is unkeyed (LOW/informational; matches spec). FIXED on `audit/full-2026-07-02` as an opt-in: `src/ui/config-audit-cache.ts` now derives each chain link via `chainDigest`, which uses HMAC-SHA256 keyed with `AUDIT_CACHE_HMAC_SECRET` when that env var is set (making the chain genuinely tamper-EVIDENT — only a secret-holder can forge a valid chain), and falls back to the original unkeyed sha256 when it is absent (backward-compatible; existing chains and tests unchanged). Unit tests added: same-secret verifies, keyed≠unkeyed, changed-secret → integrity fails, no-secret → sha256 fallback. Git remains the authority (spec §8); this only strengthens the accelerator. FOLLOW-UP (deployment, not code): to actually enable tamper-evidence, set `AUDIT_CACHE_HMAC_SECRET` on the dashboard host (fly secret) and document it alongside `AUDIT_TOTP_SECRET`/`AUDIT_GIT_WRITE_TOKEN`; note that turning it on invalidates verification of any pre-existing unkeyed chain (expected). `src/ui/config-audit-cache.ts` chains plain `sha256(prevHash + canonical(entry))` with no server secret, so anyone able to write the JSONL file can recompute a fully valid chain from genesis (forge history), not merely trip the integrity flag. This is an accepted design per spec §8 (git is the authority; the cache is a best-effort accelerator), so no action is strictly required — but if the cache is ever relied on for tamper-evidence, key the chain with an HMAC over a dedicated server secret. Recorded for visibility.

- [ ] [origin:audit:full:2026-07-02] [status:open] OPERATIONAL (not a code finding): a real-looking GitHub PAT is present in plaintext in the working tree at `.dispatch-token` / `.dispatch-token.local`. Both are gitignored and untracked (confirmed `git ls-files` — NOT committed, no repo leak), but the live credential on disk was surfaced during the audit and should be rotated as a precaution. Left untouched per audit constraints.
