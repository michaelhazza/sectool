...
- **Implements:** §10 harness core — corpus directory walker (`corpus/static/<RULE-ID>/{vulnerable,clean}/`), EXPECTED.json comparison, per-rule + aggregate recall/precision accounting, **rule-inventory ↔ corpus-directory cross-check** (CI fails any rule/check lacking both fixtures), non-zero exit on any miss or any rule without fixtures. **Single canonical id source:** `benchmark/run.ts` exports a `CANONICAL_CHECK_IDS` constant — the 11 custom rules (§7.1) + 3 wrapped scanner families (semgrep/gitleaks/osv) + the enumerated `LIVE-*` checks (§7.3) + the wildcard families `ZAP-P-*`/`ZAP-A-*`/`NUCLEI-*` at family granularity (NOT per upstream template, preserving §11's locked granularity). The corpus cross-check (P6-1) AND the docs-completeness check (P6-3) both import this one list, so no builder mints ids from docs/fixtures/scanner-output independently. This is the test-first substrate for P3+. [external review MEDIUM-2.]
...
- **Test-first/G1 acceptance:** unit tests over the pool/timeout/family-status accounting with fake scanner functions (pure orchestration logic; no real git/network in the unit test — the real shallow-clone contract is exercised by P6-1's repo-acquisition integration test against a local bare-git fixture, MEDIUM-3). `npx eslint` + targeted vitest.
...
- **Phase:** P4
- **Files:** `src/live/ratelimit.ts` (§11 inventory — plan-gap 2 RESOLVED); test colocated
...
- **Phase:** P6
- **Files:** extend `benchmark/run.ts`; complete `benchmark/corpus/**` coverage cross-check; `benchmark/live-fixture` integration into the harness; `npm run benchmark` wiring; **repo-acquisition integration test** `src/static/orchestrator.integration.test.ts` (local bare-git fixture, no network)
- **Implements:** §10 — full recall (100% target) / precision (0 FP) accounting per rule + aggregate over the real corpus and the live fixture (scanned ONLY via the benchmark-scoped allowlist, shipped config untouched); rule-inventory ↔ corpus cross-check now covers all 11 rules + 3 wrapped families + each live `checkId` family, driven by the P1-5 `CANONICAL_CHECK_IDS` list (MEDIUM-2); non-zero exit on any miss/missing-fixture. **Repo-acquisition contract (§6.2) integration-tested here** (MEDIUM-3): against a local bare git repo fixture (no network), assert `localPath:null` performs a shallow single-branch clone (`--depth 1`) into a fresh temp dir, records the scanned commit SHA into `target.commit`, does NOT initialise submodules, and reuses no prior temp state.
...
- **Implements:** §11 docs granularity — id, rationale, fix guidance + code example (sourced into remediation packs §5.3), fixture links; one doc per family (NOT per upstream template). The 11 custom-rule docs already landed with their rules in P3. **Docs-completeness check imports the same P1-5 `CANONICAL_CHECK_IDS` list** (MEDIUM-2) and asserts exactly one `docs/rules/<id>.md` per canonical id — same source of truth as the P6-1 corpus cross-check, so coverage and docs can never disagree on the id set.
...
- **Phase:** P7
- **Files:** `ui/**` (Vite + React 18 + TS SPA): shared severity/run-status color tokens + plain-language vocabulary layer, routing, and the 6 screens — Portfolio overview (`index.html` shape), Run report (`run-report.html`), Finding detail (`finding-detail.html`), Fixes (`fixes.html`), Trends (`trends.html`), Targets & safety (`targets.html`); built assets shipped with the package; **`ui/src/vocabulary.ts`** (greppable single-source export of the §5.2 normative label mapping) and **`docs/ui-prototype-mapping.md`** (one row per locked prototype screen → its React screen/component + key data source) [external review MEDIUM-4]
...
- **Test-first/G1 acceptance:** per `docs/spec-context.md` framing, frontend component tests are `none_for_now` — acceptance is shape-match to the locked prototypes + plain-language vocabulary presence, verified by the pr-reviewer/mockup reference at review time, plus `npx eslint` on `ui/**`. **Reviewer-auditable artifacts (grep-checkable, no component-test suite — honours the `none_for_now` posture):** `ui/src/vocabulary.ts` exists and exports every §5.2 normative term, and `docs/ui-prototype-mapping.md` has one row per locked prototype screen mapping it to its React component + data source [external review MEDIUM-4]. (No new Vitest suite required for SPA components; G2 build covers the Vite build.) See plan-gap 6.
- **Depends on:** P7-1.
...
2. **RESOLVED — §11 amended (commit cfbb9e9).** `src/live/ratelimit.ts` (§4.5 aggregate-per-host serializer), `src/live/auth.ts` (§6.2 carrier-aware login), and `src/report/lock.ts` (§14 workspace lock) are now named filenames in the spec §11 inventory, so the plan's 1:1 mapping holds and all three are in-scope build targets with no operator confirmation outstanding. (Inlining each into every consumer would duplicate a safety-critical invariant across files; one module each is correct.) [external review HIGH-1 confirmed the amendment is the right resolution.]
