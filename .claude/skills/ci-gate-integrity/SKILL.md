...
- Silent-override registries: a registry (HTTP routes, event handlers, DI bindings, strategy names) that silently accepts a later registration claiming a key an earlier one already holds leaves the earlier guard/handler dead — it never fires again, while everything still looks wired (compiles, is registered, is tested). A dedicated detector for this class must track its registration set DERIVED FROM SOURCE (scan the code for registration sites), never hand-maintained — a hand-kept list is exactly the drift this gate exists to catch. Reference: `scripts/gates/verify-duplicate-registrations.mjs` (warning-first, `guard-ignore*: duplicate-registrations` suppression grammar).
