...
| 4 | `tasks/builds/<slug>/` | Build dirs whose branch/PR is merged or deleted and untouched > `buildArchiveDays` (default 30), OR any dir carrying an `archive_deferred:` marker in its `progress.md` | **Sync-before-move (W3a): a merged build's card must be on the board before its record leaves `tasks/builds/`, or it strands.** **Audit mode:** REPORT the eligible dirs and any `archive_deferred: <outcome> — <reason>` markers only — read-only, NEVER run status sync (it mutates the external board) and NEVER move. **Apply mode:** for each candidate, first verify merged state via `git log`/`gh pr list`, then run `node scripts/status/sync-status.mjs --slug <slug> --expect-status MERGED` and move the dir to `tasks/builds/_archive/<slug>/` ONLY when that run's target outcome satisfies the archive-eligibility table (`applied`/`equivalent`/`created`, or `absent` for infra-only board-unreachability); on `stale_conflict`/`refused`/`partial` re-record the `archive_deferred:` marker and leave the dir in place. Record each result in the sweep report. Skip the slug named in `tasks/current-focus.md`; never archive `_example/`. |
