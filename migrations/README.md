...
Per-version migration scripts that run automatically during `/claudeupdate` on consuming repos. The pattern is modelled on Rails / Drizzle / Flyway database migrations: every framework version that needs operator-visible cleanup or per-repo file changes ships a script here; `scripts/run-migrations.cjs` discovers them and runs in order.
...
`scripts/run-migrations.cjs` is called with `(consumerRoot, fromVersion, toVersion)`:
